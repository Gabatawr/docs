# Архитектура динамических LoRA для AI
*(Технологическая реализация многоуровневого сознания)*
...
## Позиция в стеке Wavekit

Архитектура Dynamic LoRA занимает позицию **Слоя 1.5** в стеке Wavekit:

```
Слой -1: Pulse Origin
Слой  0: Язык системы
Слой  1: PWA (одиночный предиктивный агент)
Слой 1.5: Dynamic LoRA (коллективная адаптация) ← настоящее положение
Слой  2: SOD (среда, вибрация, временная материализация)
Слой  3: Temporal Growth
Слой  4: World Intersection
```

### Связь с PWA (Слой 1)
- **Context и Agent уровни** Dynamic LoRA работают как микро-адаптация **внутри** каждого PWA-агента.
- PWA предоставляет архитектуру предикта, рефлекса, согласователя — Dynamic LoRA добавляет непрерывное обучение весов поверх неё.

### Связь с SOD (Слой 2)
- **Swarm уровень** Dynamic LoRA — это общая память роя, которая становится частью вибрационной среды SOD.
- В терминах SOD: Dynamic LoRA — это механизм, через который «кубы» обмениваются состоянием не через explicit message passing, а через имплицитное искривление parameter space.

### FREE/SCREENED duality
Двухрежимная природа параметров (FREE — свободная дисперсия, SCREENED — экранированная стабильность) является операциональным аналогом дуальности экранирования и дисперсии в космологической модели. Различие: в Dynamic LoRA это механизм управления пластичностью весов.

> **Примечание:** Философский вход в тему — узел 21 «Лора-слой» в канонической последовательности Wavekit Stack. Настоящий документ — его инженерная реализация.
...
Temporal scale слоя — не константа, заданная извне. Это emergent свойство системы.
...
temporal_scale — не параметр конфигурации, а вычисляемое свойство системы
```
...
Вклады агентов не просто суммируются — они взаимодействуют (интерферируют).
